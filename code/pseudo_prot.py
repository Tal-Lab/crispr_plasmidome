...
def plasmids(cutoff):
    ...
    all_proteins_csv = f'{tables}/pseudo_proteins_90.csv'
    if not os.path.isfile(all_proteins_csv) or os.stat(all_proteins_csv).st_size == 0:
        ...
        df_proteins = pd.read_csv(all_proteins_csv, sep = ',', header = 0, index_col = 0)
    ...
    matches_within_proteins_csv = f'{tables}/all_plasmids_pseudo_matches_within_proteins_{cutoff}.csv'
